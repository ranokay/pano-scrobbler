...
public struct ScrobbleTimingPreferences: Codable, Equatable, Sendable {
    ...
    public func nextScrobbleDelay(duration: TimeInterval?, alreadyPlayed: TimeInterval?) -> TimeInterval? {
        if let duration, duration > 0, duration < Double(minimumDurationSeconds) {
            return nil
        }
        ...
        return max(targetDelay - max(alreadyPlayed ?? 0, 0), 0)
    }

    public func scrobbleDelay(duration: TimeInterval?, alreadyPlayed: TimeInterval?) -> TimeInterval {
        nextScrobbleDelay(duration: duration, alreadyPlayed: alreadyPlayed) ?? 0
    }
}
